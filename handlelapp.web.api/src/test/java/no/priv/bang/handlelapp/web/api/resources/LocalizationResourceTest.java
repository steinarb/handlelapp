...
class LocalizationResourceTest {
    ...
    void testAvailableLocales() {
        var handlelapp = mock(HandlelappService.class);
        when(handlelapp.defaultLocale()).thenReturn(NB_NO);
        when(handlelapp.availableLocales()).thenReturn(Arrays.asList(Locale.forLanguageTag("nb-NO"), Locale.UK).stream().map(l -> LocaleBean.with().locale(l).build()).toList());
        var resource = new LocalizationResource();
        resource.handlelapp = handlelapp;
    ...
    void testDisplayTextsForDefaultLocale() {
        ...
        var displayTexts = (Map<String, String>)resource.displayTexts(handlelapp.defaultLocale().toString()).getEntity();
